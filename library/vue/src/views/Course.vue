<template>
  <div>
    ...
    <el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'"
      ...
      <el-table-column prop="patientId" label="病人名称"></el-table-column>
      <el-table-column prop="name" label="科室名称"></el-table-column>
      <el-table-column prop="number" label="挂号单号"></el-table-column>
      <el-table-column prop="times" label="问诊时间"></el-table-column>
      <el-table-column prop="doctor" label="接诊医生"></el-table-column>
    ...
    <el-dialog title="问诊信息" :visible.sync="dialogFormVisible" width="30%" >
      <el-form label-width="80px" size="small">
        ...
        <el-form-item label="接诊医生">
          <el-select clearable v-model="form.doctorId" placeholder="请选择">
            <el-option v-for="item in doctors" :key="item.id" :label="item.nickname" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    selectCourse(courseId) {
      this.request.post('/course/PatientCourse/' + courseId + "/" + this.user.id).then(res => {
        if (res.code === '200') {
